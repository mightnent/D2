      ...
      if scores is None:
        ...
        if not skip_labels:  
          ...
          if not agnostic_mode:
            ...
            if display_str == 'robot':
                mv.Motor_Forward()
            elif display_str == 'obstacle' or display_str == 'bottle':
                mv.Motor_TurnRight()          
            #other implementations of movement logic after stage B.  
	    #looking to use lidar and Robot.Operating.system to complement computer vision. 
	    #cv alone is a bit hard to guage distance.  	
            #print(display_str)
        if not skip_scores:
          if not display_str:
